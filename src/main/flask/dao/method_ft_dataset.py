...
if __name__ == '__main__':
    ...
    # 加载数据集
    data = PaperDataset('../data/AI2024', "TI", "WC", "AB", "ab_seq")
    ft_dataset = []
    try:
        for i in range(0, len(data), 2):
            if (data[i].r_result and data[i].r_result) != '':
                ft_dataset.append({
                    ...
                    .format(TI=data[i].TI, r_method=data[i].r_method,r_result=data[i].r_result,WC=data[i].WC)
                })
        ...
        pass
    # 写出到Json文件
    with open('../data/method_ft_dataset.json', 'w', encoding='utf-8') as file:
